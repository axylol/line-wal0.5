...
std::vector<const char *> GL_FUNCS = {
    "glUniform4iARB",
    "glTexSubImage3D",
    "glDrawElements",
    "glSetFenceNV",
...
void unresolvedError(const char *name)
{
...
#include "skibidigfx-inl.h"

// End of auto generated

void *skibidigfx_resolve(const char *name)
{
...
void opengl::load_gl_funcs()
{
    skibidigfx_initialize_real_funcs();
    for (int i = 0; i < GL_FUNCS.size(); i++)
    {
        void *sym = Line::DlSym(NULL, GL_FUNCS[i]);
        if (!sym)
            continue;
        void *h = skibidigfx_resolve(GL_FUNCS[i]);
        if (!h)
            continue;
